...
module('Acceptance | helpers | calendarCenter', function (hooks) {
  ...
  test('`calendarCenter` invokes the `@onCenterChange` action of the target component WITH the selector we pass it', async function (assert) {
    ...
    assert
      ...
      .hasText('September 2013', 'The nav component has updated');
    assert
      .dom('.calendar-center-2-calendar [data-date="2013-09-01"]')
      .exists('The days component has updated');
  });
  ...
  test('`calendarCenter` works even if the calendar is tagless', async function (assert) {
    ...
    assert
      .dom('.calendar-center-4 .ember-power-calendar-nav-title')
      .hasText('September 2013', 'The nav component has updated');
    assert
      .dom('.calendar-center-4 [data-date="2013-09-01"]')
      .exists('The days component has updated');
  });
});
...
module('Acceptance | helpers | calendarSelect', function (hooks) {
  ...
  test('`calendarSelect` selects the given date ', async function (assert) {
    ...
    assert
      ...
      .hasClass('ember-power-calendar-day--selected', 'The 11th of October is selected');
  });
  ...
  test('`calendarSelect` selects the given date changing the month center on the process', async function (assert) {
    ...
    assert
      ...
      .hasClass('ember-power-calendar-day--selected', 'The 3rd of september is selected');
  });
  ...
  test('`calendarSelect` selects the given date in wormhole', async function (assert) {
    ...
    assert
      .dom('.dropdown-content [data-date="2013-10-15"]')
      .hasClass('ember-power-calendar-day--selected', 'The 15th is selected');
    await calendarSelect('.dropdown-content', new Date(2013, 8, 3));

    assert
      .dom('.dropdown-content .ember-power-calendar-nav-title')
      .hasText('September 2013', 'The nav component has updated');
    assert
      .dom('.dropdown-content [data-date="2013-09-01"]')
      .exists('The days component has updated');
    assert
