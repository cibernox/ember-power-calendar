...
test('`calendarCenter` invokes the `onCenterChange` action of the target component WITH the selector we pass it', async function(assert) {
  ...
  assert.equal(find('.calendar-center-2-calendar .ember-power-calendar-nav-title').textContent.trim(), 'October 2013');
  await calendarCenter('.calendar-center-2-calendar', new Date(2013, 8, 3));

  assert.equal(
    find('.calendar-center-2-calendar .ember-power-calendar-nav-title').textContent.trim(),
    'September 2013',
    'The nav component has updated'
  );
  assert.ok(find('.calendar-center-2-calendar [data-date="2013-09-01"]'), 'The days component has updated');
});
...
test('`calendarCenter` throws an error if called on a calendar without `onCenterChange` action', async function(assert) {
  ...
  assert.equal(find('.calendar-center-3 .ember-power-calendar-nav-title').textContent.trim(), 'October 2013');
...
test('`calendarCenter` works even if the calendar is tagless', async function(assert) {
  ...
  assert.equal(
    find('.calendar-center-4 .ember-power-calendar-nav-title').textContent.trim(),
    'September 2013',
    'The nav component has updated'
  );
  assert.ok(find('.calendar-center-4 [data-date="2013-09-01"]'), 'The days component has updated');
});
...
test('`calendarSelect` selects the given date changing the month center on the process', async function(assert) {
  ...
  assert.equal(
    find('.calendar-select-1 .ember-power-calendar-nav-title').textContent.trim(),
    'September 2013',
    'The nav component has updated'
  );
  assert.ok(find('.calendar-select-1 [data-date="2013-09-01"]'), 'The days component has updated');
  assert.ok(
    find('.calendar-select-1 [data-date="2013-09-03"]').classList.contains('ember-power-calendar-day--selected'),
