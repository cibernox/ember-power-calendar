...
import { add, normalizeDate, normalizeCalendarValue } from '../utils.js';
import PowerCalendarNav from './power-calendar/nav.js';
import PowerCalendarDays from './power-calendar/days.js';
import { precompileTemplate } from '@ember/template-compilation';
import { setComponentTemplate } from '@ember/component';
import { g, i, n } from 'decorator-transforms/runtime-esm';

class PowerCalendar extends Component {
  static {
  ...
  get navComponent() {
    return this.args.navComponent || PowerCalendarNav;
  }
  get daysComponent() {
    return this.args.daysComponent || PowerCalendarDays;
  }
  ...
  static {
    setComponentTemplate(precompileTemplate("{{#let (this.calendarAPI this.publicAPI (hash Nav=(component this.navComponent calendar=this.publicAPI isDatePicker=@isDatePicker) Days=(component this.daysComponent calendar=this.publicAPI isDatePicker=@isDatePicker autofocus=@autofocus))) as |calendar|}}\n  {{#let (element this.tagWithDefault) as |Tag|}}\n    <Tag class=\"ember-power-calendar\" role={{if @isDatePicker \"dialog\" \"group\"}} aria-modal={{if @isDatePicker \"true\"}} aria-label={{if @ariaLabel @ariaLabel (unless @ariaLabeledBy \"Choose Date\")}} aria-labelledby={{@ariaLabeledBy}} ...attributes id={{calendar.uniqueId}}>\n      {{#if (has-block)}}\n        {{yield calendar}}\n      {{else}}\n        <calendar.Nav />\n        <calendar.Days />\n      {{/if}}\n    </Tag>\n  {{/let}}\n{{/let}}", {
      strictMode: true,
      scope: () => ({
        hash,
        element
